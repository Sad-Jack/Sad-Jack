<!-- ╔══════════════════════════════════════════════════════════════╗ -->
<!-- ║                        HEADER / BANNER                         ║ -->
<!-- ╚══════════════════════════════════════════════════════════════╝ -->

<div align="center">

<a href="https://github.com/Sad-Jack">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,100:7aa2f7&height=210&section=header&text=Jack%20Ripper&fontSize=60&fontColor=ffffff&fontAlignY=34&desc=Konstantin%20%E2%80%A2%20QA%20Automation%20%E2%86%92%20Machine%20Learning%20%2F%20AI&descAlignY=56&descSize=18&animation=fadeIn" alt="banner" />
</a>

<a href="https://readme-typing-svg.demolab.com">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3200&pause=700&color=7AA2F7&center=true&vCenter=true&width=680&lines=QA+Automation+Engineer+%C2%B7+4%2B+years;Systematically+moving+into+Machine+Learning+%2F+AI;Creator+of+the+ML+Road+project;Data+%E2%86%92+hypotheses+%E2%86%92+metrics+%E2%86%92+conclusions" alt="typing" />
</a>

<br/>

[![Telegram](https://img.shields.io/badge/Telegram-@JACK__R__I__P-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/JACK_R_I_P)
[![ML Road](https://img.shields.io/badge/ML_Road-channel-229ED9?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/ML_Road)
[![Stepik](https://img.shields.io/badge/Stepik-course-1A75BB?style=for-the-badge&logo=stepik&logoColor=white)](https://stepik.org/course/281722/promo)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Konstantin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/konstantin-semionov-367171340/)
[![Email](https://img.shields.io/badge/Email-yandex-FC3F1D?style=for-the-badge&logo=maildotru&logoColor=white)](mailto:sad.sadsad.sad@yandex.ru)

[🇷🇺 Русский](README.md)  ·  🇬🇧 **English**

![profile views](https://komarev.com/ghpvc/?username=Sad-Jack&style=flat&color=7aa2f7&label=PROFILE+VIEWS)

</div>

---

## 🧭 Navigation

[About me](#-about-me) · [ML Road](#-ml-road-project) · [Moving into ML](#-moving-into-ml) · [Stack](#-current-stack) · [Experience](#-experience--results) · [Projects](#-projects) · [Certificates](#-certificates) · [Contacts](#-contacts)

---

## 👋 About me

I'm a **QA Automation Engineer** with 4+ years of experience testing web, mobile and backend systems.

**Main focus:**
- automation of API, UI, DB and integration scenarios
- testing microservice architecture
- analysis of logs, traces and business events
- formalizing requirements and turning vague tasks into verifiable scenarios

I currently work at **SENSE group**: automating backend and UI checks in Kotlin, testing service-to-service integrations, verifying Kafka events, validating data in PostgreSQL and MongoDB, and investigating incidents via Jaeger and OpenSearch.

In parallel, I'm systematically moving into **Machine Learning** — and I share the journey openly in the ML Road project.

---

## 🚀 ML Road project

> **ML Road** — my project where I teach ML and AI from scratch in plain language, without overload and without the "learn calculus first" gatekeeping.

| | |
|---|---|
| 📚 **Free course on Stepik** | 99 lessons, from basics to AI/LLM — [open](https://stepik.org/course/281722/promo) |
| 📱 **Telegram channel** | learning diary, notes and breakdowns — [@ML_Road](https://t.me/ML_Road) |
| 💬 **Community chat** | questions & discussion — [ML_Road_Chat](https://t.me/ML_Road_Chat) |
| 🧪 **Pet project** | Customer Churn Prediction — [repository](https://github.com/Sad-Jack/customer-churn-prediction) |

My QA background helps me look at ML from an engineering angle: data, metrics, pipelines, testing and model behavior in production.

---

## 🤖 Moving into ML

Machine Learning isn't a decorative line for me — it's a dedicated growth track.

**Near-term focus:**
- Python for working with data
- NumPy, Pandas, basic EDA
- classical ML approaches and simple pipelines
- pet projects that show not just code but the line of thinking: `data → hypotheses → metrics → conclusions`

I'm drawn to the intersection of **QA, data and ML**: quality validation, testing data-driven systems and engineering work around ML products.

---

## 🛠 Current stack

**Languages & fundamentals**

![Languages](https://skillicons.dev/icons?i=kotlin,java,py,bash,html,css)

**Development tools**

![Dev Tools](https://skillicons.dev/icons?i=idea,androidstudio,vscode,gradle,maven,postman)

**Testing & infrastructure**

![Testing & Infra](https://skillicons.dev/icons?i=selenium,gherkin,docker,jenkins,git,github,gitlab,linux)

**Databases & observability**

![Databases & Observability](https://skillicons.dev/icons?i=postgres,mongodb,elasticsearch,grafana)

**Also:** JUnit5 · Selenide · REST Assured · Cucumber · WireMock · Kafka · Swagger · DBeaver · Jaeger · TestOps · Jenkins · GitLab CI/CD

---

## 📊 GitHub stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Sad-Jack&show_icons=true&hide_border=true&count_private=true&include_all_commits=true&theme=tokyonight&title_color=7aa2f7&icon_color=7aa2f7" alt="stats" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Sad-Jack&layout=compact&hide_border=true&langs_count=8&theme=tokyonight&title_color=7aa2f7" alt="top langs" />

<img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=Sad-Jack&hide_border=true&theme=tokyonight&ring=7aa2f7&fire=7aa2f7&currStreakLabel=7aa2f7" alt="streak" />

<img src="https://github-profile-trophy.vercel.app/?username=Sad-Jack&theme=tokyonight&no-frame=true&no-bg=true&margin-w=4&column=7" alt="trophies" />

</div>

---

## 💼 Experience & results

### 🔹 SENSE group · `Sep 2025 — present`
- Automation of backend and UI checks in Kotlin, JUnit5, Gradle, REST Assured, Selenide
- Testing microservice integrations, Kafka events, data in PostgreSQL and MongoDB
- Analysis of cross-service calls via Jaeger, logs and events via OpenSearch
- WireMock and Proxyman for dependency isolation and traffic analysis

### 🔹 AVOSEND · `Jul 2024 — Sep 2025`
- Test base for Web, Mobile, API and DB
- Stack: Kotlin, Spring Boot, JUnit5, Selenide, REST Assured, JDBC, Cucumber
- Improved autotest architecture and reduced tech debt
- Turned informal tasks into documentation and verifiable results

### 🔹 SberTech · `Aug 2023 — Jul 2024`
- End-to-end UI + API scenarios in Java, Maven, JUnit, Selenium, REST Assured
- Integration of checks into Jenkins, work with Docker and Selenoid
- Test documentation standards, PR review

### 🔹 AppEvent · `Mar 2022 — Aug 2023`
- Autotests from scratch in Python, Selenium, PyTest, Allure
- Testing mobile and web products, API and business logic via Postman, Swagger, DBeaver

**What I bring to a team**
- I strengthen quality not only with tests but with clarity in requirements
- I find defects at the seams between services, data and business logic
- I improve readability and maintainability of the test base through refactoring
- I understand first — then automate

---

## 📂 Projects

**🤖 ML / AI**
- [Customer Churn Prediction](https://github.com/Sad-Jack/customer-churn-prediction) — customer churn prediction
- [Team of Agents](https://github.com/Sad-Jack/Team-of-agents) — multi-agent system
- [EasyManageBot](https://github.com/Sad-Jack/EasyManageBot) — Telegram bot powered by an AI agent
- [ML Road](https://github.com/Sad-Jack/ML_Road) — knowledge base and offline ML/AI handbook

**🧪 QA Automation**
- [Java + REST Assured + Maven](https://github.com/jackripper2703/Java-Rest-Assured)
- [Python + Selenium](https://github.com/jackripper2703/Selenium_test_project)
- [Selenium Grid + Allure (SimbirSoft)](https://github.com/jackripper2703/SimbirSoft)
- [PartyPlanner (Kotlin)](https://github.com/Sad-Jack/PartyPlanner)

---

## 🎓 Certificates

<details>
<summary><b>Expand certificate gallery</b></summary>

<br/>

[![java_intermediate](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/java_intermediate.jpg)](https://www.sololearn.com/en/certificates/CC-NV6X6H3J)
[![curs_selenium_java](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/curs_selenium_java.jpg)](https://stepik.org/cert/2466534)
[![curs_api_rest_assured](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/curs_api_rest_assured.jpg)](https://stepik.org/cert/2427706)
[![curs_selenium_python](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/curs_selenium_python.jpg)](https://stepik.org/cert/2093169)
[![curs_html_css](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/curs_html_css.jpg)](https://stepik.org/cert/2440785)
[![introduction_to_java](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/introduction_to_java.jpg)](https://www.sololearn.com/en/certificates/CC-OCZMJC4D)
[![curs_kotlin](https://github.com/Sad-Jack/Sad-Jack/raw/main/dist/curs_kotlin.jpg)](https://stepik.org/cert/2900074)

</details>

---

## 📫 Contacts

- **Telegram:** [@JACK_R_I_P](https://t.me/JACK_R_I_P)
- **ML Road channel:** [@ML_Road](https://t.me/ML_Road)
- **LinkedIn:** [konstantin-semionov](https://www.linkedin.com/in/konstantin-semionov-367171340/)
- **Email:** <sad.sadsad.sad@yandex.ru>

---

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://Sad-Jack.github.io/Sad-Jack/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://Sad-Jack.github.io/Sad-Jack/github-contribution-grid-snake.svg" />
  <img alt="snake animation" src="https://Sad-Jack.github.io/Sad-Jack/github-contribution-grid-snake.svg" />
</picture>

<br/><br/>

[![Codewars](https://www.codewars.com/users/Sad-Jack/badges/large)](https://www.codewars.com/users/Sad-Jack)

<br/>

> My main growth vector is **Machine Learning**: learning, building projects and steadily building a career in this direction.

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:7aa2f7,100:1a1b27&height=120&section=footer" alt="footer" />

</div>
